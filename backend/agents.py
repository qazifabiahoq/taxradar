...
async def run_analysis(extracted_text: str) -> dict:
    try:
        ...
        classifier_raw = await run_agent(document_classifier, extracted_text)
        ...
        validator_raw, deduction_raw, missing_raw = await asyncio.gather(
            run_agent(income_validator, parallel_input),
            run_agent(deduction_scorer, parallel_input),
            run_agent(missing_document_detector, parallel_input),
        )
        ...
        final_raw = await run_agent(report_synthesizer, synthesizer_input)
